...
def closer_city(lat, lon, city1, city2):
    ...
    'Bucharest'
    """
    city1_dist = sqrt((get_lat(city1)-lat)**2 + (get_lon(city1)-lon)**2)
    city2_dist = sqrt((get_lat(city2)-lat)**2 + (get_lon(city2)-lon)**2)
...
def pascal(row, column):
    ...
    if column > len(triangle(row)):
        ...
        return triangle(row)[row][column]
